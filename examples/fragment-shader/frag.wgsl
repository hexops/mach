...
@stage(fragment) fn main(
    ...
    // This will give the circular effect to our shader
    let dist = length(uv - vec2<f32>(0.5,0.5)) * 3.14159;

    // We multiply dist by the frequency of the waves, 
    // then we add the speed at which they move times the current time 
    // + 1) / 2 is so that h remains inside the [0,1] range
    let h = (sin(16.0 * dist - 5.0 * ubo.time) + 1.0) / 2.0;
    let s = 1.0;
    let l = 0.4;

    return vec4(hsl_to_rgb(h * 360.0,s,l), 1.0);
}

// Standard hsl to rgb formula
