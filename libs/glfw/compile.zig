...
fn addGLFWIncludes(step: *std.build.LibExeObjStep) void {
    step.addIncludePath(sdkPath("/zig-deps/upstream/glfw/include"));
    step.addIncludePath(sdkPath("/zig-deps/upstream/vulkan_headers/include"));
}

fn addGLFWSources(b: *Builder, lib: *std.build.LibExeObjStep, options: Options) std.mem.Allocator.Error!void {
    const include_glfw_src = comptime "-I" ++ sdkPath("/zig-deps/upstream/glfw/src");
    switch (lib.target_info.target.os.tag) {
        ...
        .macos => lib.addCSourceFiles(&.{
            ...
            try flags.append(comptime "-I" ++ sdkPath("/zig-deps/upstream/glfw/src"));
