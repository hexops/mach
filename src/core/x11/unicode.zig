...
pub fn unicodeFromKeySym(keysym: c.KeySym) ?u21 {
    ...
    if ((keysym >= 0x0020 and keysym <= 0x007e) or
        (keysym >= 0x00a0 and keysym <= 0x00ff))
    {
    ...
    if ((keysym & 0xff000000) == 0x01000000) {
        return @intCast(keysym & 0x00ffffff);
    }

    // Binary search in table
    while (max >= min) {
        mid = (min + max) / 2;
        if (keysym_table[mid][0] < keysym) {
            min = mid + 1;
        } else if (keysym_table[mid][0] > keysym) {
            max = mid - 1;
        } else {
            return keysym_table[mid][1];
        }
    }
